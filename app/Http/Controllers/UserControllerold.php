        ...
        return datatables()->of($industry)
                        ...
                        ->addColumn('post_closing', function($row) {
                         ...
                         ->addColumn('remaining', function($row) {
                               $pay2 =  DB::table('postoffice_payments')->where('created_at','>=',date('Y-m-d 00:00:00',strtotime($row->payment_date)))->where('created_at','<=',date('Y-m-d 23:59:00',strtotime($row->payment_date)))->where('customer_id',$row->customer_id)->first();
                            if($pay2 != '' && $row->opening_balance != ''){
                                $remaining =  $row->opening_balance-$pay2->opening_balance;
                            }else{
                                $remaining = '';
                            }
                            $remaining = '';
    ...
    public function monthlylist(Request $request) {
        ...
        return datatables()->of($industry)
                        ...
                        ->addColumn('amountt', function($row) {
                            $pay = DB::table('payments')->where('payment_date','>=',date('Y-m-d',strtotime('01-'.Session::get('currentmonth'))))->where('payment_date','<=',date('Y-m-d',strtotime('31-'.Session::get('currentmonth'))))->where(array('customer_id'=>$row->id))->orderBy('id','DESC')->sum('amount');
                            if($pay != ''){
                        ...
                        ->addColumn('opening', function($row) {
                            $pay = DB::table('payments')->where('payment_date','>=',date('Y-m-d',strtotime('01-'.Session::get('currentmonth'))))->where('payment_date','<=',date('Y-m-d',strtotime('31-'.Session::get('currentmonth'))))->where(array('customer_id'=>$row->id))->orderBy('id','DESC')->first();
                            if($pay != ''){
                        ...
                        ->addColumn('closing', function($row) {
                               $pay2 = DB::table('payments')->where('payment_date','>=',date('Y-m-d',strtotime('01-'.Session::get('currentmonth'))))->where('payment_date','<=',date('Y-m-d',strtotime('31-'.Session::get('currentmonth'))))->where('customer_id',$row->id)->orderBy('id','DESC')->first();
                            if($pay2 != ''){
                         ...
                         ->addColumn('post_amount', function($row) {
                            $p_pay = DB::table('postoffice_payments')->where('payment_date','>=',date('Y-m-d',strtotime('01-'.Session::get('currentmonth'))))->where('payment_date','<=',date('Y-m-d',strtotime('31-'.Session::get('currentmonth'))))->where(array('customer_id'=>$row->id))->orderBy('id','DESC')->sum('amount');
                         ...
                         ->addColumn('post_opening', function($row) {
                            $pay = DB::table('postoffice_payments')->where('payment_date','>=',date('Y-m-d',strtotime('01-'.Session::get('currentmonth'))))->where('payment_date','<=',date('Y-m-d',strtotime('31-'.Session::get('currentmonth'))))->where(array('customer_id'=>$row->id))->orderBy('id','DESC')->first();
                            if($pay != ''){
                        ...
                        ->addColumn('post_closing', function($row) {
                               $pay2 = DB::table('postoffice_payments')->where('payment_date','>=',date('Y-m-d',strtotime('01-'.Session::get('currentmonth'))))->where('payment_date','<=',date('Y-m-d',strtotime('31-'.Session::get('currentmonth'))))->where(array('customer_id'=>$row->id))->orderBy('id','DESC')->first();
                            if($pay2 != ''){
                        ...
                        ->addColumn('payment_month', function($row) {
                                 $pay21 = DB::table('postoffice_payments')->where('payment_date','>=',date('Y-m-d',strtotime('01-'.Session::get('currentmonth'))))->where('payment_date','<=',date('Y-m-d',strtotime('31-'.Session::get('currentmonth'))))->where(array('customer_id'=>$row->id))->orderBy('id','DESC')->first();
                                if($pay21 != ''){
